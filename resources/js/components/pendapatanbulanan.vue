<template>
    <div>
        ...
        <table cellpadding="0" cellspacing="0">
            ...
            <tr >
                <td class="text-center" colspan="6"><h3>Laporan Pendapatan Bulanan</h3></td>
            </tr>
            <table class="table">
                <tr  >
                <th>
            ...
            <tr  v-for="(lapor,index) of laporan" v-bind:key="lapor['key']">
                ...
                <td v-if="lapor.service !== null">
                    {{lapor.service}}
                </td>
                <td v-else>
                    0
                </td>
                <td v-if="lapor.penjualan !== null">
                    {{lapor.penjualan}}
                </td>
                <td v-else>
                    0
                </td>
                <td v-if="lapor.total !== null" class="text-right">
                    {{lapor.total}}
                </td>
...
export default {
   ...
   created(){
    ...
    methods: {
        ...
        async showTransaksi(){
            this.tampung = await Controller.getpendapatanbulanan(this.$route.params.id);
            this.tahun = this.$route.params.id;
            this.laporan = this.tampung.data;
            this.jumlah=this.laporan.reduce((acc,item)=> acc+item.total,0);
